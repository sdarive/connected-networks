...
const FixedNetworkApp = () => {
  ...
  const getCharactersForCategory = (category) => {
    ...
    const allCharacters = Object.values(characterData.categories).flatMap(cat => cat.characters);
    ...
    return allCharacters.filter(char => {
      const charCategory = char.category || getCharacterCategory(char);
  ...
  // Helper function to determine character category
  const getCharacterCategory = (character) => {
    // Find which category this character belongs to
    for (const [categoryKey, categoryData] of Object.entries(characterData.categories)) {
      if (categoryData.characters.some(char => char.id === character.id)) {
...
export default FixedNetworkApp;
